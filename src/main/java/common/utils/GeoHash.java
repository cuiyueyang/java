...
public class GeoHash {
     ...
     * @param keyLength GeoHash值长度
     */
    public GeoHash(int keyLength, int latMultiple, int lonMultiple) {
        if (keyLength <= 0 || keyLength > SUPPORTED_MAX_KEY_LENGTH) {
            throw new IllegalArgumentException("the keyLength not supported");
        }
        init(keyLength, latMultiple, lonMultiple);
    }
    ...
    private void init(int keyLength, int latMultiple, int lonMultiple) {
        this.keyLength = keyLength;
        int totalBit = keyLength * BASE_32_ENCODE_BIT;
        this.lngBit = ((totalBit + 1) >> 1) - lonMultiple;
        this.latBit = (totalBit >> 1) - latMultiple;
        if (this.lngBit < 0 || this.latBit < 0) {
