...
/**
 * 程序描述: Base64编码算法的一种实现。提取自JODD源码，速度更快。
 */
...
public final class Base64Util
{
     ...
     * 用户未指定编码时默认使用的编码.
     */
    private static final String _DEF_ENCODING = "UTF-8" ;
    ...
    public static char[] encodeToChar( String s, String encoding )
    {
        if( null == encoding )
            encoding = _DEF_ENCODING ;
        try
        {
            return encodeToChar( s.getBytes( encoding ), false );
    ...
    public byte[] decode( char[] arr )
    {
        ...
        {
            int i = INV[arr[sndx++]] << 18 | INV[arr[sndx++]] << 12 | INV[arr[sndx++]] << 6
                    | INV[arr[sndx++]];
    ...
    public static byte[] encodeToByte( String s, String encoding )
    {
        if( null == encoding )
            encoding = _DEF_ENCODING ;
        try
    ...
    {
        return encodeToString( s, _DEF_ENCODING ) ;
    }

    public static String encodeToString( String s, String encoding )
    {
        if( null == encoding )
            encoding = _DEF_ENCODING ;
        try
    ...
    {
        return decodeToString( s, _DEF_ENCODING ) ;
    }

    public static String decodeToString( String s, String encoding )
    {
        if( null == encoding )
            encoding = _DEF_ENCODING ;
        try
